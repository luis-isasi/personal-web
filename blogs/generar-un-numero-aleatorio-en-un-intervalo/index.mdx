...
Ahora con `Math.random()`, podemos aprovechar eso para convertirlo a un porcentaje y multiplicarlo por el número de posibilidades, tomando el ejemplo anterior de 3 y 6, las posibildades serian 4, si lo muliplicamos por decimal generado entre 0 y 1, es decir que el resultado de esta multiplicación sería menor que 4 y mayor que 0, entonces te estarás preguntando "y para qué quiero un decimal aleatorio entre 0 a 4", pues ahora vamos a las siguientes líneas de código.
...
Un ejemplo con 3 y 6:
```
function intervalNum(min, max) {
  const posibilidades =  max - min + 1; // posibilidades = 6 - 3 + 1 =  4
  let random = Math.random() * posibilidades; // random = 0.7 * 4 = 2.8
  random = Math.floor(decimal); // random = 2
  return min + random; // output: 3 + 2 = 5
}
```
Con esta explicación quiero tu creatividad crezca y que sepas que todo es posible para los programadores.
